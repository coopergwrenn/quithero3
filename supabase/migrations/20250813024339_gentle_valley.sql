/*
  ...
  1. New Tables
    - `deposit_contracts`
      ...
      - `commitment_days` (integer, commitment period)
      - `start_date` (date)
      - `end_date` (date)
  ...
  2. Security
    ...
    - Add policies for users to manage own financial data
*/
...
CREATE INDEX IF NOT EXISTS idx_deposit_contracts_user_status 
  ON deposit_contracts(user_id, status);
CREATE INDEX IF NOT EXISTS idx_savings_challenges_user_active 
  ON savings_challenges(user_id, completed);